...
C.C.Claw is a lightweight, TypeScript-based CLI agent framework that brings Claude Code-style interactive terminal experience to any LLM API. It features onboarding-first configuration, 19 built-in tools, Vim mode, and a Soul system for persistent agent identity.

C.C.Claw 是一个轻量级、基于 TypeScript 的 CLI Agent 框架，将 Claude Code 风格的交互式终端体验带入任意 LLM API。支持首次启动引导、19 个内置工具、Vim 模式和持久化 Agent 灵魂系统。
...
首次运行时，C.C.Claw 会自动弹出配置向导。按以下步骤配置：
...
```bash
cclaw chat                    # 启动交互式对话（默认模型）
cclaw chat -m MiniMax-M2.7    # 指定模型
cclaw chat -p                 # 管道模式（stdin → stdout）
cclaw chat -v                 # 详细模式（显示完整工具调用）

cclaw onboard                 # 手动启动配置向导
cclaw doctor                  # 诊断配置问题
cclaw init                    # 初始化模板文件
cclaw logs                    # 查看日志
cclaw logs -f                 # 实时跟踪日志
cclaw mcp list                # 查看 MCP 服务器
cclaw plugin list             # 查看插件
cclaw update                  # 检查更新
cclaw update --check          # 仅检查，不更新
```

### 4. REPL 命令

在 `cclaw chat` 中输入斜杠命令：
...
修改权限：编辑 `~/.cclaw/permissions.json`
...
编辑：`/soul-edit` 或直接编辑 `~/.cclaw/SOUL.md`
...
```bash
cclaw mcp list                              # 列出已配置 MCP 服务器
cclaw mcp add my-server /path/to/server     # 添加 MCP 服务器
cclaw mcp remove my-server                  # 删除 MCP 服务器
```

MCP 配置文件保存在 `~/.cclaw/.mcp.json`
...
On first run, C.C.Claw automatically launches the setup wizard. Follow these steps:
...
All configurations are stored in `~/.cclaw/`:

```
~/.cclaw/
├── .cclaw.json       # Main config (provider, model, endpoint)
...
```bash
# Main commands
cclaw chat                    # Start interactive chat (default model)
cclaw chat -m MiniMax-M2.7    # Specify model
cclaw chat -p                 # Pipe mode (stdin → stdout)
cclaw chat -v                 # Verbose mode (show full tool calls)

# Management
cclaw onboard                 # Launch setup wizard
cclaw doctor                  # Diagnose configuration
cclaw init                    # Initialize template files
cclaw logs                    # View logs
cclaw logs -f                 # Follow logs (tail -f)

# Integrations
cclaw mcp list                # List MCP servers
cclaw plugin list             # List plugins
cclaw update                  # Check for updates
```
...
C.C.Claw includes 19 pre-built tools, categorized by permission level:
...
Edit `~/.cclaw/permissions.json` to customize tool permissions.
...
Edit via `/soul-edit` or directly edit `~/.cclaw/SOUL.md`.
...
```bash
cclaw mcp list                              # List configured servers
cclaw mcp add my-server /path/to/server     # Add an MCP server
cclaw mcp remove my-server                  # Remove an MCP server
```

MCP configuration is stored in `~/.cclaw/.mcp.json`
