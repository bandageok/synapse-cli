...
| 结构化终端信息流 | `v0.4.0`、`v0.5.0` | 用结构化事件和逐轮视图替代前缀字符串，加入折叠工具摘要、CJK 响应式布局、命令面板和审批面板 |
| 限流恢复 | `v0.5.1` | 按 `Retry-After` 可取消重试 429；交互任务持续等待，脚本默认有界，并且不消耗 Agent turn budget |
| 产品事实与运行时一致性 | `v0.6.0` | 接通默认/exec 入口、真实模型切换、原子会话、可信用量展示和插件未激活边界，新增 ADR-0010 与回归测试 |
| 公开版本事实一致性 | `v0.6.1` | 同步中英文入口、路线图与验证证据，并用发布门禁阻止 package、runtime、changelog 和 README 版本漂移 |
...
registered tool
  ...
  -> tool implementation
  -> strict sandbox in auto, host shell in approved ask/full-access
```
...
- `ask`：写入、执行、网络和敏感文件读取要求明确审批；
- `auto`：从不询问，只执行工作区安全能力；找不到可运行的 Bubblewrap/Docker 时返回错误，不退回宿主 shell；
- `full-access`：从不询问并使用宿主 shell，但仍经过 Schema、deny list、危险命令、路径、MCP 与网络防线；
- MCP 命令、脚本内容或能力指纹变化：撤销信任；
- `HEARTBEAT.md` 中的代码块：只作为文本，不执行。

这套设计仍不是绝对安全保证。它是可测试的纵深防御，并明确承认宿主权限、容器配置和第三方依赖仍属于攻击面。

权限模式由共享 `PermissionManager` 持有，而不是分别缓存在 REPL、ToolRegistry 和 Bash 中。`synapse permissions set` 设置后续会话默认值，`--permission-mode`/`--yolo` 只覆盖一次启动，`/permissions` 原子切换当前会话的审批策略与 Shell 隔离方式。这个拆分借鉴了 Codex 将 approval policy 与 sandbox policy 分开的思路，但保留了 Synapse 自己的集中工具校验边界。
