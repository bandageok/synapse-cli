...
// Downsample to 25x25 (each cell = 2x2 avg)
var dW = 25, dH = 25;
var sX = W / dW, sY = H / dH;
var dst = [];
...
// Use half-block rendering: 2 rows -> 1 line with \u2580 (top) and \u2584 (bottom)
// But Ink doesn't support combining two colors in one char well with simple Text.
// Strategy: Output 25 rows but each row uses 25 chars at full block for dark, or space for light
// Actually: just use the 25x25 grid with block chars + Ink colors
...
ts += '        } else {\n';
ts += '          elements.push(React.createElement(Text, { key: c + "-" + y + "-" + i, color: c as any, bold: c === "black" }, "\\u2588".repeat(count)));\n';
ts += '        }\n';
ts += '        i += count;\n';
ts += '      }\n';
